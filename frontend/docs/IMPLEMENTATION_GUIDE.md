...
| -------------------- | --------------------------------------------------- | ------------------------------------------------------------ | -------------------------------------------- | -------------------------------------------- |
| `ResultPage`         | `result`, `isFromHistory`, `onBack`                 | 戻る遷移（`/record` or `/history`）                          | 中間結果ページの導線制御                     | 統合音域計算、詳細比較表示                   |
...
1. `/record` → `/record/normal` で録音し、`/result` 表示後に「トップへ戻る」で `/record` へ戻る。
2. `/record` → `/record/upload` で解析し、タイムアウト/通信断時のエラーメッセージを確認する。
