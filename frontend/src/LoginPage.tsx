/**
 * 【LoginPage.tsx】
 * 役割：Google認証を使ってアプリにログインするための画面です。
 * 💡 設計図（FRONTEND_STRUCTURE.md）に基づく移動案：
 * 1. 移動先: src/features/auth/pages/LoginPage.tsx
 * 2. 理由：認証（Auth）という特定の機能ドメインに関連する画面であるためです。
 */

import React from "react";
// 認証状態やログイン用関数を管理するコンテキストを使用します
import { useAuth } from "./contexts/AuthContext";

const LoginPage: React.FC = () => {
  // AuthContext から Googleログインを実行する関数を取得します
  const { loginWithGoogle } = useAuth();

  return (
    /**
     * ── ログインカードのレイアウト ──
     * 背景を透過させた（backdrop-blur）サイバーなデザインのカードを中央に配置します。
     */
...
// React.memo により、不要な再レンダリングを防止してパフォーマンスを最適化します。
