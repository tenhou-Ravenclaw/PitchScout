...
/**
 * useItemCollection - お気に入り・履歴などのコレクション管理を共通化するカスタムフック
 *
...
/** コレクション操作の結果型 */
type ItemCollectionResult<T extends number[] | string[]> = {
  /** コレクションIDリスト */
  ids: T;
  /** 初回ロード中フラグ */
  loading: boolean;
...
): ItemCollectionResult<number[]>;
export function useItemCollection(
  type: "favoriteArtist"
): ItemCollectionResult<number[]>;
export function useItemCollection(
  type: "analysisHistory"
): ItemCollectionResult<string[]>;
export function useItemCollection(
  type: CollectionType,
  onLoginRequired?: () => void
): ItemCollectionResult<number[]> | ItemCollectionResult<string[]> {
  ...
  useEffect(() => {
    ...
    collectionApi[type].get()
      ...
      .catch((err) => {
        showToast(toUserMessage(err, "コレクション取得に失敗しました"));
      })
    ...
    async (id: number | string, name?: string) => {
      ...
      try {
        if (type === "favoriteSong") {
          ...
          if (wasIncluded) {
            await collectionApi.favoriteSong.remove(songId);
          } else {
            await collectionApi.favoriteSong.add(songId);
          }
        } else if (type === "favoriteArtist") {
          const artistId = id as number;
          if (wasIncluded) {
            await collectionApi.favoriteArtist.remove(artistId);
          } else {
            await collectionApi.favoriteArtist.add(artistId, name ?? "");
          }
        } else if (type === "analysisHistory") {
          // 履歴は削除のみ（追加は録音フローで行う）
          await collectionApi.analysisHistory.remove(id as string);
          setIds((prev) => (prev as string[]).filter((i) => i !== id));
        }
      } catch (err) {
        showToast(toUserMessage(err, "コレクション操作に失敗しました"));
        // ロールバック（曲・アーティストのみ）
        if (type === "favoriteSong" || type === "favoriteArtist") {
          const numId = id as number;
          setIds((prev) =>
